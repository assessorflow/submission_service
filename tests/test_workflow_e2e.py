...
async def _create_assessment(client: AsyncClient) -> dict:
    resp = await client.post("/api/v1/assessments", json={
        ...
        "groups": [
            {"group_name": "Group A", "members": ["student1@e2e.com", "student2@e2e.com"]},
        ],
    })
...
async def test_generate_and_review(client: AsyncClient):
    ...
    await repo.update_assessment_status(assessment_id, "processing", workflow_id=workflow_id)

    qs = await repo.create_question_set(workflow_id)
    await repo.write_generated_questions(qs["id"], [
        {"question_type": "structured", "content": "What is encapsulation?",
         "structured_answer": "A", "metadata": {"options": {"A": "Bundling", "B": "Looping"}},
         "iteration": 1},
        {"question_type": "non_structured", "content": "Explain OOP.",
         "non_structured_model_answer": "OOP uses objects.", "metadata": {"max_marks": 10},
         "iteration": 1},
    ])
...
async def test_approve_all(client: AsyncClient):
    ...
    qs = await repo.create_question_set(workflow_id)
    await repo.write_generated_questions(qs["id"], [
        {"question_type": "structured", "content": "Q1?", "structured_answer": "A",
         "metadata": {}, "iteration": 1},
        {"question_type": "structured", "content": "Q2?", "structured_answer": "B",
         "metadata": {}, "iteration": 1},
    ])
    ...
    approved_resp = await client.get(f"/api/v1/assessments/{assessment_id}/approved-questions")
...
async def test_approve_with_removal(client: AsyncClient):
    ...
    await repo.update_assessment_status(assessment_id, "processing", workflow_id=workflow_id)
    qs = await repo.create_question_set(workflow_id)
    written = await repo.write_generated_questions(qs["id"], [
        {"question_type": "structured", "content": "Good Q", "structured_answer": "A",
         "metadata": {}, "iteration": 1},
        {"question_type": "structured", "content": "Bad Q", "structured_answer": "B",
         "metadata": {}, "iteration": 1},
    ])
    bad_id = written[1]["id"]
...
async def test_invite_and_participate(client: AsyncClient):
    ...
    qs = await repo.create_question_set(workflow_id)
    await repo.write_generated_questions(qs["id"], [
        {"question_type": "structured", "content": "What is OOP?",
         "structured_answer": "A",
    ...
    approved = await client.get(f"/api/v1/assessments/{assessment_id}/approved-questions")
    ...
    access_resp = await client.get(f"/api/v1/participate/{assessment_id}?token={participant_id}")
    assert access_resp.status_code == 200
    assert access_resp.json()["status"] == "ready"
    assert access_resp.json()["total_questions"] == 3  # from assessment config (2 structured + 1 non-structured)

    # Participant gets questions — no answers exposed
    q_resp = await client.get(f"/api/v1/participate/{assessment_id}/questions?token={participant_id}")
...
async def test_evaluation_and_reports(client: AsyncClient):
    ...
    await repo.write_generated_questions(qs["id"], [
        {"question_type": "structured", "content": "Q1?", "structured_answer": "A",
         "metadata": {"options": {"A": "Yes", "B": "No"}}, "iteration": 1},
    ])
    await client.post(
        f"/api/v1/assessments/{assessment_id}/review/approve",
        json={"removed_question_ids": []},
    ...
    await client.post(f"/api/v1/assessments/{assessment_id}/invite", json={"participant_ids": []})
    ...
    evaluation = await repo.create_evaluation(workflow_id, participant_id, submission["id"])
    await repo.write_evaluation_details(
        evaluation["id"],
        [{"question_id": question_id, "score": 10.0, "max_score": 10.0,
          "evaluation_method": "deterministic"}],
        total_score=10.0, max_score=10.0,
    )
    ...
    report = await repo.create_report(
        ...
        report_content={
            ...
            "per_question_feedback": [
                {"question_id": question_id, "score": 10, "max_score": 10, "feedback": "Correct!"},
            ],
    ...
    report_resp = await client.get(f"/api/v1/reports/{report['id']}?token={participant_id}")
    assert report_resp.status_code == 200
    assert report_resp.json()["status"] == "completed"

    # Distribute reports
    dist_resp = await client.post(f"/api/v1/assessments/{assessment_id}/reports/distribute")
